...
class Board {
	...
	public:
		...
		array< int, 5 > RandomDice(int numbers_to_generate){
			...
			sleep(0.00001);
			auto system_time_nanoseconds = std::chrono::duration_cast<std::chrono::nanoseconds>(
        		std::chrono::system_clock::now().time_since_epoch()
    		).count();
			std::mt19937 rng(system_time_nanoseconds);
			system_time_nanoseconds *= static_cast<int>(pow(system_time_nanoseconds, 2));
